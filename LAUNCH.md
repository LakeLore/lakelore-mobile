...
**ENGINEERING POST-LAUNCH (in the plan, Rounds 11–16 + carryovers):** canonical provenance column (schema bump) · UA self-ID with O10 outreach · MB reactivation (product-hold, licence-clean; recipe in registry `productHold` note) · MB paired abundance×trophy rating scatter (design pending owner) · v1.0-public NE 400 (accepted; superseded at release) · coverage-change companions (paywall/About one-liners, RC promo-entitlement goodwill playbook) · review poller (needs an ASC API `.p8` key — owner) · optional real-device Play screenshots swap.
