...
**County picker** (`src/components/CountyMapPicker.tsx`): geometry for 48 US states generated by `scripts/generateCountyPaths.mjs` into `src/data/countyPaths/` (keys match each state's canonical `lakes.county` values). States without matching census geometry (RI towns, AK areas, Canadian FMZs/regions) fall back to a list-only picker fed by the server's `/filters` counties.
