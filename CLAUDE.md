...
2. `SearchScreen` — species + lake-name search, advanced filters, list or scatter view. The in-search state switcher (`StatePickerModal`) uses the same map picker. Filter ranges + the scatter toggle are gated per state by the generated capability flags (`hasCpue/hasLength/hasCatch/hasStocking` in `src/generated/states.ts`).
