...
1. `StateSelectScreen` — pan/zoom SVG **map of the US + Canada** (2026-07-15 all-states launch; `src/components/StateMapPicker.tsx`, geometry generated by `scripts/generateStatePaths.mjs` into `src/data/statePaths.ts`), plus a grouped A-Z list below the map as the accessible path. 48 states/provinces are active (43 US + ON/BC/MB/SK/AB; the 8 with no stocking AND no CPUE data — GA SC AZ CA MA DE RI QC — are registry `active:false`, drawn muted, not selectable) — `src/activeStates.ts` derives `ACTIVE_STATES` from the generated registry export (`src/generated/states.ts`, regenerate with `node ~/lakelore-data/bin/export-mobile-states.js`). MN is free; every other state opens in **preview mode** for non-subscribers (see paywall section). Picking a state ALWAYS opens the county selector next (SearchScreen auto-opens it on every state change — the old "first visit only" gating was removed).
...
- **Free**: MN. **Paid**: every other active state/province (42 US + 5 CA) via `LakeLore All-States` subscription at **$5.99/yr** auto-renewing. Active states derive from `lakelore-data/registry/states.json` on both server and app.
